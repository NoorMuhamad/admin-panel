...
const List = memo(({ data, totalPages, isLoading, page, limit, handlePageChange, setFormModeAndVisible, handleDeletePackage, handleSortBy }) => {
	...
	const columns = [
		...
		renderColumn('Monthly Fee', 'monthlyFee'),
		renderColumn('Speed', 'speed'),
		renderColumn('is Active', 'isActive'),
		{
			title: renderEllipsisTooltip('Created Date', 150),
			dataIndex: 'createdAt',
			width: 150,
			key: 'createdAt',
		...
		},
		{
			...
			render: (_, record) => (
				<Space size="middle">
					...
					<DeleteOutlined onClick={() => handleDeletePackage(record.id)} style={{ color: 'red' }} />
				</Space>
			),
		},
	];
